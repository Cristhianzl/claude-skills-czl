...
description: Generate Langflow-shaped PR titles, commit messages, and descriptions from staged changes or a branch diff — conventional-commit format with Langflow scopes (auth, components, graph, api, frontend, bundles/<name>, lfx, docs, ci), Fixes #N allowed in description, no Co-Authored-By trailer, version bumps via make patch v=X.Y.Z, BUNDLE_API.md callout when bundles change, breaking-change ! flag when a Component class rename or public surface change is involved. Use when the user asks to create a PR, write a PR title, generate a commit message, or draft a PR description for Langflow. Output goes directly to chat — never to a file; never run git commit/add/push.
license: MIT
---
